...
void Chess::fenToBoard(const std::string& fen){
    ...
    for (int i = 0; i < fen.length(); i++){
        ...
        if(c == '/'){
            ...
            switch (std::tolower(c)) { //figure out what piece it is
                case 'p':
                    piece = Pawn;
                    break;
                case 'n': 
...
bool Chess::canKingsideCastle(char color){
    if (color == 'W' && !w_kingMoved && !w_kingRookMoved){ //check has not moved
        if (pieceNotation(stateString().c_str(), 7,5) == "0" && pieceNotation(stateString().c_str(), 7,6) == "0"){ //check that inbetween squares are empty
            
            if (!isUnderAttack(stateString().c_str(), 7,4,'B') && !isUnderAttack(stateString().c_str(), 7,5,'B') && !isUnderAttack(stateString().c_str(), 7,6,'B')){ // check that king isn't in check and is not castling into check
                return true;
            }
        }
    }
    if (color == 'B' && !b_kingMoved && !b_kingRookMoved){ //check has not moved
        
        if (pieceNotation(stateString().c_str(), 0,5) == "0" && pieceNotation(stateString().c_str(), 0,6) == "0"){ //check that inbetween squares are empty
            if (!isUnderAttack(stateString().c_str(), 0,4,'W') && !isUnderAttack(stateString().c_str(), 0,5,'W') && !isUnderAttack(stateString().c_str(), 0,6,'W')){ // check that king isn't in check and is not castling into check
                return true;
            }
        }
    }
...
bool Chess::canQueensideCastle(char color){
    if (color == 'W' && !w_kingMoved && !w_kingRookMoved){ //check has not moved
        ...
        if (pieceNotation(stateString().c_str(), 7,1) == "0" && pieceNotation(stateString().c_str(), 7,2) == "0" && pieceNotation(stateString().c_str(), 7,3) == "0"){ //check that inbetween squares are empty

            if (!isUnderAttack(stateString().c_str(), 7,4,'B') && !isUnderAttack(stateString().c_str(), 7,1,'B') && !isUnderAttack(stateString().c_str(), 7,2,'B') && !isUnderAttack(stateString().c_str(), 7,3,'B')){ // check that king isn't in check and is not castling into check
                return true;
            }
        }
    ...
    if (color == 'B' && !b_kingMoved && !b_kingRookMoved){ //check has not moved
        ...
        if (pieceNotation(stateString().c_str(), 0,1) == "0" && pieceNotation(stateString().c_str(), 0,2) == "0" && pieceNotation(stateString().c_str(), 0,3) == "0"){ //check that inbetween squares are empty
            if (!isUnderAttack(stateString().c_str(), 0,4,'W') && !isUnderAttack(stateString().c_str(), 0,1,'W') && !isUnderAttack(stateString().c_str(), 0,2,'W') && !isUnderAttack(stateString().c_str(), 0,3,'W')){ // check that king isn't in check and is not castling into check
                return true;
            }
        }
    }
...
#if defined(UCI_INTERFACE)
std::string bestUCIMove = bestMove.from;
bestUCIMove += bestMove.to;
...
void Chess::UCIMove(const std::string& move) {
    ...
    int toCol = move[2] - 'a';
    int toRow = '8' - move[3];
    //uciInterface.UCILog(std::to_string(fromCol) + ',' + std::to_string(fromRow) + " to " + std::to_string(toCol) + ',' + std::to_string(toRow));
    BitHolder& src = getHolderAt(fromRow, fromCol);
    BitHolder& dst = getHolderAt(toRow, toCol);
    Bit* bit = src.bit();
    if (bit) {
        //uciInterface.UCILog("hit");
        if (dst.bit()) {
            pieceTaken(dst.bit());
        }
        dst.dropBitAtPoint(bit, ImVec2(0, 0));
